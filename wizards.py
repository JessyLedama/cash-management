...
class staff_claim_payments(models.TransientModel):
    ...
    def action_post(self):
        setup = self.env['cash.management.general.setup'].search([
                                                                 ('id', '=', 1)])
        if setup.staff_claims_payable_account.id == False:
            raise ValidationError(
                '\'Staff Claims Payable Account\' field must have a value in General Setup!')
        else:
...
class BatchPostPettyCashWizard(models.TransientModel):
    _name = "batch.post.petty.cash.wizard"

    @api.multi
    def batch_post(self):
        active_ids = self.env.context.get('active_ids', [])
        for record in self.env['cash.management.petty.cash.header'].browse(active_ids):
            record.signal_workflow('send_for_posting')
            # record.action_post()

class BatchPostBankCashTransfers(models.TransientModel):
    _name = 'batch.post.bank.cash.transfers'

    @api.multi
    def batch_post(self):
        active_ids = self.env.context.get('active_ids', [])
        for record in self.env['cash.management.bank.transfer.header'].browse(active_ids):
            record.signal_workflow('send_for_posting')
            # record.action_post()