...
class PettyCash(models.Model):
    ...
    date = fields.Date(readonly=True, states={'draft': [('readonly', False)], 'approved':[('readonly', False)]})
    payment_method = fields.Selection([
        ('cash',"Cash"),
        ('cheque',"Cheque")], 
        default = 'cash', readonly=True, states={'draft': [('readonly', False)]})
    cheque_no = fields.Char(readonly=True, states={'draft': [('readonly', False)]})
    paying_bank = fields.Many2one('res.bank', readonly=True, states={'draft': [('readonly', False)], 'approved':[('readonly', False)]})
    payment_to = fields.Char(readonly=True, states={'draft': [('readonly', False)]})
    payment_narration = fields.Text(readonly=True, states={'draft': [('readonly', False)]})
    cashier = fields.Many2one('res.users', default = lambda self:self.env.user, readonly=True, states={'draft': [('readonly', False)]})
    ...
    tax_amount = fields.Float()
    total_amount = fields.Float(compute = 'calculate_amounts')
    line_ids = fields.One2many('cash.management.petty.cash.lines','header_id', readonly=True, states={'draft': [('readonly', False)], 'approved':[('readonly', False)]})
...
class bank_transfer_header(models.Model):
    ...
    name = fields.Char(string = "No.")
    date = fields.Date(readonly=True, states={'draft': [('readonly', False)]})
    paying_bank_journal_id = fields.Many2one('account.journal', domain=[('type','in',['cash','bank'])], string="Paying Bank Journal")
    paying_bank = fields.Many2one('res.bank', readonly=True, states={'draft': [('readonly', False)]})
    ...
    paying_bank_currency_id = fields.Many2one('res.currency', string="Currency")
    receiving_bank_journal_id = fields.Many2one('account.journal', domain=[('type','in',['cash','bank'])], string="Receiving Bank Journal")
    ...
    receiving_bank_currency_id = fields.Many2one('res.currency', string="Currency")
    ...
    amount_lcy = fields.Float(readonly=True, states={'draft': [('readonly', False)]}, string="Amount (LCY)")
    amount_fcy = fields.Float(readonly=True, states={'draft': [('readonly', False)]}, string="Amount (FCY)")
    remarks = fields.Text(readonly=True, states={'draft': [('readonly', False)]})
    account_move_ids = fields.One2many('account.move','bank_transfer_id')
    ...
    @api.onchange('paying_bank_journal_id', 'receiving_bank_journal_id','amount','conversion_rate')
    def get_currency(self):
        self.paying_bank_currency_id = self.paying_bank_journal_id.currency.id or self.paying_bank_journal_id.default_credit_account_id.company_id.currency_id.id
        self.receiving_bank_currency_id = self.receiving_bank_journal_id.currency.id or self.paying_bank_journal_id.default_debit_account_id.company_id.currency_id.id
        if self.paying_bank_journal_id and self.receiving_bank_journal_id:
            if self.conversion_rate == 0: #only compute conversion rate if no manual input
                self.conversion_rate = self.paying_bank_currency_id._get_conversion_rate(self.paying_bank_currency_id, self.receiving_bank_currency_id)
            if self.paying_bank_journal_id.currency:
                self.amount_lcy = self.conversion_rate * self.amount
            else:
                self.amount_lcy = self.amount

            if self.receiving_bank_journal_id.currency:
                self.amount_fcy = self.amount_lcy * self.conversion_rate
            else:
                self.amount_fcy = self.amount

        # get other details for backward compatibility
        if self.paying_bank_journal_id:
            self.paying_bank = self.env['res.partner.bank'].search([('journal_id','=',self.paying_bank_journal_id.id)]).bank.id
            self.receiving_bank = self.env['res.partner.bank'].search([('journal_id','=',self.receiving_bank_journal_id.id)]).bank.id
            self.get_account()
    ...
    def action_post(self):
        ...
        setup = self.env['cash.management.general.setup'].search([], limit=1)
        ...
        journal = self.env['account.journal'].search([('id','=',paying_bank.journal_id.id)]) #get journal id
        paying_journal_id = self.paying_bank_journal_id
        receiving_journal_id = self.receiving_bank_journal_id
        # company_currency = self.paying_bank_journal_id.default_debit_account_id.company_id.currency
        #period
        period = self.env['account.period'].search([('state','=','draft'),('date_start','<=',today),('date_stop','>=',today)])
        period_id = period.id
        ...
        # move_line_vals.append((0, 0,{
        #     # 'journal_id':receiving_journal_id.id,
        #     'period_id':period_id,
        #     'date':today,
        #     'name':'Bank Transfer',
        #     'account_id':self.receiving_bank_journal_id.default_debit_account_id.id,
        #     'debit': self.amount if self.receiving_bank_journal_id.currency == self.paying_bank_journal_id.currency and self.receiving_bank_journal_id.currency != self.receiving_bank_journal_id.default_debit_account_id.company_id.currency_id else self.amount * self.conversion_rate, # perform conversion at destination if the 2 currencies transacting are different
        #     'credit':0,
        #     'amount_currency' : self.receiving_bank_currency_id.compute(self.amount, self.receiving_bank_currency_id) if self.receiving_bank_journal_id.currency else 0,
        #     'currency_id': self.receiving_bank_currency_id.id if self.receiving_bank_journal_id.currency else False
        #     }))
        ...
        #     'name':'Bank Transfer',
        #     # 'account_id':dr,
        #     'account_id' : setup.outstanding_receipt_payment_account_id.id,
        #     'debit':0,
        #     'credit': self.amount if self.receiving_bank_journal_id.currency == self.paying_bank_journal_id.currency and self.receiving_bank_journal_id.currency != self.receiving_bank_journal_id.default_debit_account_id.company_id.currency_id else self.amount * self.conversion_rate,
        #     'amount_currency' : self.paying_bank_currency_id.compute(-self.amount, self.receiving_bank_currency_id)  if self.receiving_bank_journal_id.currency else 0,
        #     'currency_id': self.receiving_bank_currency_id.id if self.receiving_bank_journal_id.currency else False,
        #     }))
        ...
        move_line_vals.append((0, 0,{
            ...
            'name':'Bank Transfer',
            'account_id':self.receiving_bank_journal_id.default_debit_account_id.id,
            'debit': self.amount_lcy,# * self.conversion_rate if self.receiving_bank_journal_id.currency else self.amount,
            'credit':0,
            'amount_currency' : self.amount_fcy if self.receiving_bank_journal_id.currency else 0,
            'currency_id': self.receiving_bank_currency_id.id if self.receiving_bank_journal_id.currency else False,
            }))
        ...
        move_line_vals.append((0, 0,{
            ...
            'account_id' : setup.outstanding_receipt_payment_account_id.id,
            'debit':0,
            'credit': self.amount_lcy,# * self.conversion_rate if self.receiving_bank_journal_id.currency else self.amount,
            'amount_currency' : -self.amount_fcy if self.receiving_bank_journal_id.currency else 0,
            'currency_id': self.receiving_bank_currency_id.id if self.receiving_bank_journal_id.currency else False,
            }))
        ...
        move = journal_header.create({
            ...
            'date':today,
            'line_id': move_line_vals,
            'ref' : '%s from %s to %s' %(self.name, self.paying_bank_journal_id.name, self.receiving_bank_journal_id.name) 
            })
        
        # post source entry
        # move_line_vals2 = []
        # move_line_vals2.append((0, 0,{
        #     # 'journal_id':journal.id,
        #     'period_id':period_id,
        #     'date':today,
        #     # 'name':'Bank Transfer',
        #     'account_id': setup.outstanding_receipt_payment_account_id.id,
        #     'debit':self.amount if self.receiving_bank_journal_id.currency == self.paying_bank_journal_id.currency and self.paying_bank_journal_id.currency != self.paying_bank_journal_id.default_debit_account_id.company_id.currency_id else self.amount * self.conversion_rate,
        #     'credit':0,
        #     'amount_currency' : self.paying_bank_currency_id.compute(paying_journal_id.default_credit_account_id.company_id.currency_id.id,receiving_journal_id.currency, self.amount) if self.paying_bank_currency_id != self.receiving_bank_currency_id else 0,
        #     'currency_id': self.paying_bank_currency_id.id if self.paying_bank_journal_id.currency else False,
        #     }))
        ...
        #     'date':today,
        #     # 'name':'Bank Transfer',
        #     'account_id':self.paying_bank_journal_id.default_credit_account_id.id,
        #     'debit':0,
        #     'credit': self.amount if self.receiving_bank_journal_id.currency == self.paying_bank_journal_id.currency and self.paying_bank_journal_id.currency != self.paying_bank_journal_id.default_debit_account_id.company_id.currency_id else self.amount * self.conversion_rate,
        #     'amount_currency' : self.amount if self.receiving_bank_journal_id.currency == self.paying_bank_journal_id.currency and self.paying_bank_journal_id.currency != self.paying_bank_journal_id.default_debit_account_id.company_id.currency_id else 0
        #     }))
        ...
        move_line_vals2.append((0, 0,{
            ...
            'name':'Bank Transfer',
            'account_id': setup.outstanding_receipt_payment_account_id.id,
            'debit': self.amount_lcy,# * self.conversion_rate if self.paying_bank_journal_id.currency else self.amount,
            'credit':0,
            'amount_currency' : self.amount_fcy if self.paying_bank_journal_id.currency else 0,
            'currency_id': self.paying_bank_currency_id.id if self.paying_bank_journal_id.currency else False,
            }))
        ...
        move_line_vals2.append((0, 0,{
            ...
            'account_id':self.paying_bank_journal_id.default_credit_account_id.id,
            'debit':0,
            'credit': self.amount_lcy, # * self.conversion_rate if self.paying_bank_journal_id.currency else self.amount,
            'amount_currency' : -self.amount_fcy if self.paying_bank_journal_id.currency else 0,
            'currency_id': self.paying_bank_currency_id.id if self.paying_bank_journal_id.currency else False,
            }))

        move2 = journal_header.create({
            'journal_id':paying_journal_id.id,
            'period_id':period_id,
            'state':'draft',
            # 'name':self.name,
            'date':today,
            'line_id': move_line_vals2,
            'ref' : '%s from %s to %s' %(self.name, self.paying_bank_journal_id.name, self.receiving_bank_journal_id.name)
            })

        self.account_move_ids.write({'bank_transfer_id':move.id})
        self.account_move_ids.write({'bank_transfer_id':move2.id})

    @api.multi
    def action_view_moves(self):
        action = self.env.ref("account.action_move_journal_line").read()[0]
        action["search_view_id"] = self.env.ref("cash_management.view_account_move_filter").id
        action["context"] = {
            "search_default_bank_tranfer_id":self.id,
            # "search_view_id":self.env.ref("cash_management.view_account_move_filter").id
        }
        return action
...
class setup(models.Model):
    ...
    outstanding_receipt_payment_account_id = fields.Many2one('account.account', string="Outstanding Receipts/Payments Bal A/C")